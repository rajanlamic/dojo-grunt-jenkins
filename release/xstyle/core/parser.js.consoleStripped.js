define("xstyle/core/parser", ["xstyle/core/utils"], function(utils){
						...
						case '(': case '{': case '[':
							...
							if(operator == '{'){
								...
								if(!nextRule){
									...
									// 0 && console.warn("Unable to find rule ", selector, "existing rule did not match", nextRule.selectorText); 
								}
	...
	//							value.replace(/(?:^|,|>)\s*([\w-]+)/g, function(t, base){
			...
			function error(e){
				 0 && console.error(e.message || e, (styleSheet.href || "in-page stylesheet") + ':' + textToParse.slice(0, cssScan.lastIndex).split('\n').length);
				if(e.stack){
					 0 && console.error(e.stack);
				}			
			}
		}
	}
